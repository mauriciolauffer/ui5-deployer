...
class AdtResourceManager {
	...
	async saveResources(localResources) {
		...
		const path = BSP_PATH + "/" + encodeURIComponent(this._project.deployer.abapRepository.bspApplication);
	...
	async _getRemoteResources(path) {
		return this._adtClient.getResources(path)
			.then(response => {
				...
				return Promise.all(
					folderNodes.map(folder => this._getRemoteResources(BSP_PATH + "/" + folder.valueWithPath('atom:id')))
				);
			});
	}
