...
const BSP_PATH = "/filestore/ui5-bsp/objects";
const CTS_PATH = '/cts/transportrequests';
...
class AdtClient {
	...
	_validateAdtDiscovery(response) {
		...
		if (!this._validateAdtPath(ADT_PATH + BSP_PATH, response.body)) {
			isValid = false;
			this.logger.error(ADT_PATH + BSP_PATH + errorMessage);
		}
		if (!this._validateAdtPath(ADT_PATH + CTS_PATH, response.body)) {
			isValid = false;
			this.logger.error(ADT_PATH + CTS_PATH + errorMessage);
		}
		if (!this._validateAdtPath(ADT_PATH + PACKAGE_PATH, response.body)) {
			isValid = false;
			this.logger.error(ADT_PATH + PACKAGE_PATH + errorMessage);
		}
		if (!isValid) {
			this.logger.error("For more information, check " + ADT_PATH + "/discovery");
	...
	async _getPackage(abapPackage) {
		this.logger.info("Getting ABAP package", abapPackage);
	...
	async _getTransportRequest(transportRequest) {
		this.logger.info("Getting ABAP Transport Request", transportRequest);
		const reqOptions = {
			uri: CTS_PATH + "/" + encodeURIComponent(transportRequest) + "/consistencychecks",
			method: "POST",
			headers: {
	...
	async _getBspApplication(bspApplication) {
		this.logger.info("Getting BSP Application", bspApplication);
	...
	async _createBspApplication() {
		this.logger.info("Creating BSP Application", this._abapRepository.bspApplication);
	...
	async getResources(resourcePath) {
		...
		return this._client({ uri: resourcePath + CONTENT_PATH })
			.then((response) => {
				if (response.statusCode === 200) {
					return response;
				} else {
	...
	_getDefaultRequest(options = {}) {
		...
		const reqOptions = {
			baseUrl: options.connection.url + ADT_PATH,
			uri: "",
			gzip: true,
			jar: true,
			headers: {
